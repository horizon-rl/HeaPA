...
def _extract_text_from_prompt_field(prompt_field) -> str:
    ...
    try:
        ...
        if isinstance(prompt_field, list):
            ...
            if (
                prompt_field
                and isinstance(prompt_field[0], dict)
                and "content" in prompt_field[0]
            ):
...
class AugmentationLogger:
    ...
    def __init__(self, log_dir: str, experiment_name: str, max_buffer_size: int = 1000):
        ...
        self.augmentation_file = (
            self.log_dir / f"{experiment_name}_{self.timestamp}_augmentations.jsonl"
        )
        self.annotation_file = (
            self.log_dir / f"{experiment_name}_{self.timestamp}_annotations.jsonl"
        )
        self.pool_snapshot_file = (
            self.log_dir / f"{experiment_name}_{self.timestamp}_pool_snapshots.jsonl"
        )
        self.summary_file = (
            self.log_dir / f"{experiment_name}_{self.timestamp}_summary.json"
        )
        ...
        with open(
            self.log_dir / f"{experiment_name}_{self.timestamp}_metadata.json", "w"
        ) as f:
        ...
        with self._lock:
            ...
            if "estimated_reward" in record:
                ...
                self.stats["avg_estimated_reward"] = (
                    prev_avg * (n - 1) + record["estimated_reward"]
                ) / n
    ...
    def log_annotation(self, record: Dict[str, Any]):
        ...
        with self._lock:
            ...
            if "estimated_reward" in record and "final_reward" in record:
                ...
                if n > 0:
                    self.stats["avg_final_reward"] = (
                        sum(r["final"] for r in self.stats["reward_correlation"]) / n
                    )

            if len(self.annotation_buffer) >= self.max_buffer_size:
                self._flush_annotation_buffer()

    def log_pool_snapshot(
        self, pool_metrics: Dict[str, Any], sample: List[Dict[str, Any]] = None
    ):
        ...
        with self._lock:
            ...
            if (
                len(self.pool_snapshot_buffer) >= self.max_buffer_size // 10
            ):  # Less frequent
...
class ThreadSafeQueryPool:
    ...
    def __init__(
        ...
        self._high_heap_min: List[
            Tuple[float, int, QueryRecord]
        ] = []  # (reward, seq, rec)
        # (-reward, seq, rec)
    ...
    def capacity_remaining(self) -> int:
        with self._lock:
            return max(
                0, self._max_size - (len(self._low_heap) + self._high_size_unlocked())
            )
    ...
    def initialize_uniform(
        self, seed_items: List[QueryRecord], uniform_reward: float = 0.5
    ):
        if not seed_items:
            return
        ...
        with self._lock:
            ...
            items = (
                seed_items[-self._max_size :]
                if len(seed_items) > self._max_size
                else list(seed_items)
            )
            for it in items:
    ...
    def add_many(self, items: List[QueryRecord]):
        ...
        if added_count < len(items):
            logger.debug(f"Added {added_count}/{len(items)} items (pool at capacity)")
    ...
    def _sample_medium_k_unlocked(self, k: int) -> List[QueryRecord]:
        ...
        if actual_k >= n_total:
            ...
            if len(candidates) > 0:
                chosen_idx = self._rng.choice(
                    len(candidates), size=min(actual_k, len(candidates)), replace=False
                ).tolist()
            else:
                chosen_idx = []
        ...
        for idx in chosen_idx:
            if idx < len(keep_mask):
                keep_mask[idx] = False
        to_reinsert = [
            c for i, c in enumerate(candidates) if i < len(keep_mask) and keep_mask[i]
        ]
        origins_back = [
            origins[i]
    ...
    def _pop_high_max_unlocked(self):
        ...
        while self._high_heap_max:
            neg_r, seq, it = heapq.heappop(self._high_heap_max)  # neg_r = -reward
    ...
    def _clean_high_min_top_unlocked(self):
        while self._high_heap_min and (
            self._high_heap_min[0][1] not in self._active_high
        ):
            heapq.heappop(self._high_heap_min)

    def _clean_high_max_top_unlocked(self):
        while self._high_heap_max and (
            self._high_heap_max[0][1] not in self._active_high
        ):
    ...
    def _deep_clean_heaps_unlocked(self):
        ...
        # Clean high_heap_min
        if len(self._high_heap_min) > batch_size or len(self._high_heap_min) > 2 * len(
            self._active_high
        ):
            valid_min = []
        ...
        # Clean high_heap_max
        if len(self._high_heap_max) > batch_size or len(self._high_heap_max) > 2 * len(
            self._active_high
        ):
            valid_max = []
    ...
    def _copy_record_unlocked(it: QueryRecord) -> QueryRecord:
        return QueryRecord(
            raw_prompt_data=it.raw_prompt_data.copy()
            if isinstance(it.raw_prompt_data, np.ndarray)
            else it.raw_prompt_data,
            input_ids=it.input_ids.clone() if it.input_ids is not None else None,
            attention_mask=it.attention_mask.clone()
            if it.attention_mask is not None
            else None,
            position_ids=it.position_ids.clone()
            if it.position_ids is not None
            else None,
            gt=it.gt,
            reward=it.reward,
    ...
    def return_batch(
        ...
        priority_update: dict[str, float] | None = None,
        ema_alpha: float = 0.3,
    ) -> None:
        """
        Reinsert a subset of previously sampled items with optional EMA-smoothed
        priority updates.

        Args:
            items: Records to (re)insert. Items not passed here are effectively dropped.
            priority_update: Optional map record_id -> new priority in [-1, 1].
                            If a record_id is absent or maps to a non-finite value, its
                            priority is left unchanged.
            ema_alpha: EMA weight in [0, 1]. NewPriority = alpha*new + (1-alpha)*old.

        Behavior:
            - Priorities are clipped to [-1, 1].
            - Non-finite rewards are ignored; the item keeps its current (clipped) reward.
            - Reinserts respect capacity and your existing full-insert semantics (may evict an easy item).
            - Performs a local rebalance and a final capacity trim.
        """
        import numpy as _np
        ...
        ema_alpha = float(_np.clip(ema_alpha, 0.0, 1.0))
        pu = priority_update or {}

        with self._lock:
            for it in items:
                # Current reward (safe, clipped)
                try:
                    curr = float(it.reward)
                except Exception:
                    curr = 0.0
                if not _np.isfinite(curr):
                    curr = 0.0
                curr = float(_np.clip(curr, -1.0, 1.0))

                # Optional new priority
                newp = pu.get(it.record_id, None)
                if newp is not None and _np.isfinite(newp):
                    newp = float(_np.clip(newp, -1.0, 1.0))
                    it.reward = float(
                        _np.clip(ema_alpha * newp + (1.0 - ema_alpha) * curr, -1.0, 1.0)
                    )
                else:
                    # Keep existing (clipped) reward
                    it.reward = curr

                # Reinsert with capacity awareness
                total = len(self._low_heap) + self._high_size_unlocked()
                if total < self._max_size:
                    self._push_high_unlocked(it)
                else:
                    # Respect existing full-insert semantics (may evict highest-reward item)
                    self._insert_when_full_unlocked(it)

            # Local rebalance & final trim
            self._rebalance_low_high_unlocked(local_only=True)
            self._evict_to_capacity_unlocked()
...
class AsyncTeacherAnnotator(threading.Thread):
    ...
    def enqueue_aug(self, aug_proto: "DataProto") -> bool:
        ...
        try:
            ...
            logger.warning("Teacher annotation queue is full, applying backpressure")
            return False
    ...
    def _make_prompt(question: str) -> str:
        return (
            "You are a strict math verifier and solver, and your task is to determine if the following problem is well-posed and solvable. "
            "If solvable, compute the final numeric answer.\n\n"
            "Return ONLY a valid JSON object (no code fences, no extra text). Do NOT attempt to solve the problem step-by-step or show your work; only provide the final answer if solvable.\n"
            "Output must be a single line, exactly in this schema:\n"
            '  {"solvable": true|false, "answer": "<string or null>"}\n'
            "- Use lowercase true/false and null. Do not add fields, trailing commas, or explanations.\n"
            '- If solvable is true: put ONLY the final numeric answer as a string in "answer" (no units, no text).\n'
            '- If solvable is false: set "answer" to null.\n'
            '- If you cannot understand or parse the problem, set "solvable" to false and "answer" to null.\n'
            '- For any other issues, such as ambiguous or incomplete problems, set "solvable" to false and "answer" to null.\n'
            '- If the problem contains many noisy details, such as narrative text, explanations, or the actual problem only takes up less than half of the entire input, set "solvable" to false and "answer" to null.\n'
            "Problem:\n"
            f"{question}"
        )
    ...
    def _extract_text_and_reason(self, response) -> tuple[str, str | None]:
        ...
        try:
            if hasattr(response, "choices") and response.choices:
                ...
                if fr is None:
                    fr = getattr(getattr(ch, "finish_details", {}), "type", None) or (
                        getattr(ch, "finish_details", {}) or {}
                    ).get("type")
    ...
    @staticmethod
    def _parse_teacher_json(text: str) -> Tuple[Optional[bool], Optional[str]]:
        """Parse teacher response with better error handling."""
        t = AsyncTeacherAnnotator._strip_code_fences(text)

        # Try direct JSON parse
        try:
            obj = json.loads(t)
            solvable = obj.get("solvable", None)
            answer = obj.get("answer", None)
            if isinstance(solvable, bool) and (isinstance(answer, (str, type(None)))):
        ...
        try:
            ...
            if m:
                ...
                solvable = obj.get("solvable", None)
                answer = obj.get("answer", None)
                if isinstance(solvable, bool) and (
                    isinstance(answer, (str, type(None)))
                ):
                    return solvable, answer
        ...
        # Heuristic fallback
        num = re.search(r"[-+]?(?:\d+(?:\.\d*)?|\.\d+)(?:[eE][-+]?\d+)?", t)
        if num:
            return True, num.group(0)

        logger.warning(f"Failed to parse teacher response: {text}...")
        return None, None
    ...
    def _call_api_with_retry(
        ...
        bad_reasons = {"length", "content_filter", "max_tokens", "tool_calls"}
        for attempt in range(self.max_retries):
            try:
                ...
                if (finish_reason in bad_reasons) or (not content):
                    raw_text = (
                        content
                        if content
                        else f"[empty][finish_reason={finish_reason}]"
                    )
                    return False, None, raw_text, finish_reason

                solvable, ans = self._parse_teacher_json(content)
                is_solvable = bool(solvable) if solvable is not None else False
                return (
                    is_solvable,
                    (ans if is_solvable else None),
                    content,
                    finish_reason,
                )
    ...
    def run(self):
        ...
        while self._running:
            ...
            try:
                ...
                logger.error(f"Error processing augmented proto: {e}", exc_info=True)
                self._error_count += 1
            finally:
    ...
    def _process_augmented_proto(self, aug_proto: DataProto):
        ...
        augmented_texts = list(sub_proto.non_tensor_batch.get("raw_prompt_data", []))

        # Ensure lists have the same length as idxs
        original_texts = original_texts[: len(idxs)] + [None] * max(
            0, len(idxs) - len(original_texts)
        )
        augmented_texts = augmented_texts[: len(idxs)] + [None] * max(
            0, len(idxs) - len(augmented_texts)
        )
        ...
        for i, q in enumerate(raw_prompts):
            ...
            # --- hard filter Chinese/CJK; mark as unsolvable and skip API ---
                    ...
                    self.augmentation_logger.log_annotation(
                        {
                            ...
                            "estimated_reward": float(est[idxs[i]])
                            if idxs[i] < len(est)
                            else None,
            ...
            # Normal path: enqueue API call
            prompt = self._make_prompt(q_text)
            future = self.executor.submit(self._call_api_with_retry, prompt)
            futures.append((i, future, q_text))

        # Collect results from API calls and write them back into the preallocated slots
        for i, future, question_text in futures:
            try:
                is_solvable, ans, raw_response, finish_reason = future.result(
                    timeout=self.api_timeout * 2
                )
                teacher_solvable[i] = is_solvable
                teacher_answers[i] = ans
                teacher_raw_responses[i] = raw_response
                ...
                if self.augmentation_logger and i < len(idxs):
                    ...
                    annotation_record["finish_reason"] = finish_reason
                    self.augmentation_logger.log_annotation(annotation_record)

            except TimeoutError:
                logger.warning("API call timed out")
                teacher_solvable[i] = False
                teacher_answers[i] = None
                teacher_raw_responses[i] = ""
            except Exception as e:
        ...
        keep_idxs = np.asarray(
            [i for i, ok in enumerate(teacher_solvable) if ok], dtype=np.int64
        )
        if keep_idxs.size == 0:
...
class RayDAPOTrainer(RayPPOTrainer):
    ...
    def __init__(self, config, *args, **kwargs):
        ...
        self._annotated_inbox = deque(maxlen=2000)
        ...
        if not hasattr(self, "tokenizer") or self.tokenizer is None:
            try:
                ...
                model_name = getattr(config.model, "model_name_or_path", "gpt2")
                self.tokenizer = AutoTokenizer.from_pretrained(model_name)
                if self.tokenizer.pad_token is None:
                    self.tokenizer.pad_token = self.tokenizer.eos_token
            except Exception as e:
                logger.warning(f"Failed to initialize tokenizer: {e}. Using default.")
                self.tokenizer = None
        ...
        experiment_name = getattr(config.trainer, "experiment_name", "dapo_experiment")
    ...
    def _dump_origin_rewards(self, batch: DataProto, path: str, step: int) -> None:
        try:
            ...
            driver_rewards = list(
                batch.non_tensor_batch.get("driver_reward", [None] * len(seq_rewards))
            )
            ...
            with open(path, "a", encoding="utf-8") as f:
                for i in range(min(len(seq_rewards), len(ds), len(rids))):
                    ...
                    if i < len(origins) and origins[i] is not None:
                        ...
                        origin = (
                            "augmented"
                            if str(ds[i]) == "math_dapo"
                            else (str(ds[i]) or "train")
                        )
                    rec = {
                        "step": step,
                        ...
                        "driver_reward": (
                            float(driver_rewards[i])
                            if driver_rewards and driver_rewards[i] is not None
                            else None
                        ),
                    }
    ...
    @staticmethod
    def _parse_difficulty_strict(
        block: str, lo: float = 0.75, hi: float = 1.33
    ) -> float:
        if not block:
            return 1.0
        s = str(block).strip()
        ...
        if s.startswith("```") and s.endswith("```"):
            s = re.sub(r"^```(?:\w+)?\s*|\s*```$", "", s, flags=re.DOTALL).strip()

        # normalize comma decimals
        s = re.sub(r"(?<=\d),(?=\d)", ".", s)

        # JSON-like: {"difficulty": 0.97}
        m = re.search(r'(?i)"difficulty"\s*:\s*([-+]?(?:\d+(?:\.\d+)?|\.\d+))', s)
        if m:
    ...
    def _compute_moderate_return_plan(
        self,
        batch,
        band: float = 0.25,
        return_fraction: float = 0.20,
        min_keep: int = 1,
    ) -> tuple[set[str], dict[str, float]]:
        """
        Decide which items to RETURN (only 'moderate' ones) and compute their updated priorities.

        Args:
        band: keep items whose normalized mean return is within ±band around 0
          (after min-max scaling to [-1, 1]). If values were uniform, the
          expected keep-rate ≈ band. (e.g., band=0.25 → ~25%).
        return_fraction: cap on how many from the batch we return (0..1). The rest are dropped.
        min_keep: ensure we always keep at least this many, even if band is tight.

        Returns:
        keep_ids: set of record_ids to return
        pri_map:  record_id -> new priority in [-1, 1]
        """
        from collections import defaultdict

        import numpy as np

        # 1) Aggregate mean seq reward per record
        seq_r = batch.batch["token_level_rewards"].sum(dim=-1).detach().cpu().numpy()
        rec_ids = [str(x) for x in batch.non_tensor_batch["record_ids"]]
        agg = defaultdict(list)
        for rid, r in zip(rec_ids, seq_r):
            agg[rid].append(float(r))
        mean_r = {k: float(np.mean(v)) for k, v in agg.items()}

        # 2) Min–max normalize to [-1, 1] (low return => -1, high => +1)
        vals = np.array(list(mean_r.values()), dtype=float)
        if vals.size == 0:
            return set(), {}
        vmin, vmax = float(vals.min()), float(vals.max())
        if vmax - vmin < 1e-9:
            scaled = {k: 0.0 for k in mean_r}  # all same
        else:
            scaled01 = {k: (v - vmin) / (vmax - vmin) for k, v in mean_r.items()}
            scaled = {k: -1.0 + 2.0 * scaled01[k] for k in mean_r.keys()}

        # 3) Pick the 'moderate' band around 0 (i.e., abs(scaled) <= band)
        cand = [(k, abs(scaled[k])) for k in scaled.keys()]
        moderate = [k for k, d in cand if d <= band]

        # If the band yields none/too many, pick closest-to-0 subset
        if not moderate:
            # at least pick the single closest-to-median item
            k_best = min(cand, key=lambda kv: kv[1])[0]
            moderate = [k_best]

        # 4) Cap how many we return this step
        total_unique = len(mean_r)
        cap = (
            max(min_keep, int(round(return_fraction * total_unique)))
            if return_fraction is not None
            else len(moderate)
        )

        # sort moderates by closeness to 0 (truest 'middle'), then take up to cap
        moderate_sorted = sorted(moderate, key=lambda k: abs(scaled[k]))
        keep_ids = set(moderate_sorted[:cap])

        # 5) Priority map (what we write back to the pool)
        pri_map = {k: scaled[k] for k in keep_ids}
        return keep_ids, pri_map
    ...
    def _ensure_dataproto(self, batch_dict_or_dp) -> DataProto:
        ...
        if dp is not None and "input_ids" in dp.batch and "attention_mask" in dp.batch:
            ...
            if "raw_prompt_data" not in dp.non_tensor_batch:
                ...
                if "prompt" in dp.non_tensor_batch:
                    ...
                    for i in range(len(dp.batch["input_ids"])):
                        try:
                            ids = dp.batch["input_ids"][i].detach().cpu().numpy()
                            texts.append(self._decode_tokens_to_text(ids))
                        except Exception:
                            texts.append("")
                dp.non_tensor_batch["raw_prompt_data"] = np.asarray(texts, dtype=object)

            # Provide default driver_reward to avoid key errors when popping
            n = len(dp.batch["input_ids"])
            if "driver_reward" not in dp.non_tensor_batch:
                dp.non_tensor_batch["driver_reward"] = np.full(n, 0.5, dtype=float)
            ...
            if "origin" not in dp.non_tensor_batch:
                dp.non_tensor_batch["origin"] = np.array(["seed"] * n, dtype=object)
    ...
    def _build_dp_from_dataset_batch(self, row: Dict[str, Any]) -> DataProto:
        ...
        nt = {
            ...
            "reward_model": _normalize_reward_model_list(
                reward_model, 1, extra_info_list=[extra_info]
            ),
    ...
    def _seed_records_from_loader(self) -> List[QueryRecord]:
        ...
        reward_map_path = getattr(self.config.dynamic_data, "seed_reward_map", None)
        reward_map = (
            self._load_seed_reward_map(reward_map_path) if reward_map_path else {}
        )
        ...
        for batch_obj in self.train_dataloader:
            ...
            try:
                ...
                raw_texts = list(dp.non_tensor_batch.get("raw_prompt_data", [""] * B))
                data_srcs = list(dp.non_tensor_batch.get("data_source", ["train"] * B))
                driver_rewards = list(
                    dp.non_tensor_batch.get("driver_reward", np.full(B, 0.5))
                )
                ...
                for i in range(take):
                    ...
                    pos_ids = (
                        dp.batch.get("position_ids", [None] * B)[i]
                        if "position_ids" in dp.batch
                        else None
                    )

                    # Raw "prompt data" — keep token ids as raw source; original_text for readability
                    raw_prompt_data = input_ids.detach().cpu().numpy()
                    original_text = str(raw_texts[i]) if i < len(raw_texts) else None
                    ...
                    base_reward = (
                        float(driver_rewards[i]) if i < len(driver_rewards) else 0.5
                    )
    ...
    def _records_to_dataproto(self, recs: List[QueryRecord]) -> DataProto:
        ...
        for i, r in enumerate(recs):
            if r.input_ids is not None and r.attention_mask is not None:
                ...
                logger.warning(f"Record {i} missing required tensor fields, skipping")
        if not valid_recs:
        ...
        cfg_max = int(
            getattr(self.config.data, "max_prompt_length", max_len_in_batch or 1)
        )
        target_len = min(max_len_in_batch, cfg_max)

        truncation_mode = str(getattr(self.config.data, "truncation", "left")).lower()
        ...
        def _pad_trunc(
            ids: torch.Tensor, am: torch.Tensor
            ...
            if L > target_len:
                ...
                if left_trunc:  # left pad
                    ids2 = torch.cat(
                        [
                            torch.full(
                                (pad_n,), pad_id, dtype=ids.dtype, device=device
                            ),
                            ids,
                        ],
                        dim=-1,
                    ...
                    am2 = torch.cat(
                        [torch.zeros((pad_n,), dtype=am.dtype, device=device), am],
                        dim=-1,
                    )
                else:  # right pad
                    ids2 = torch.cat(
                    ...
                    am2 = torch.cat(
                        [am, torch.zeros((pad_n,), dtype=am.dtype, device=device)],
                        dim=-1,
                    )
            else:
                ids2, am2 = ids, am
        ...
        for r in valid_recs:
            ...
            am = (
                r.attention_mask
                if r.attention_mask is not None
        ...
        # Non-tensor metadata
        nt = {
            "raw_prompt_data": np.array(
                [r.raw_prompt_data for r in valid_recs], dtype=object
            ),
            "driver_reward": np.array(
                [float(r.reward) for r in valid_recs], dtype=np.float32
            ),
            "driver_est_reward": np.array(
                [
                    float(r.est_reward) if r.est_reward is not None else np.nan
                    for r in valid_recs
                ],
                dtype=np.float32,
            ),
            ...
            "original_text": np.array(
                [r.original_text for r in valid_recs], dtype=object
            ),
            "data_source": np.array(
                [(r.meta or {}).get("source", "train") for r in valid_recs],
                dtype=object,
            ),
            "origin": np.array(
                [(r.meta or {}).get("origin", "seed") for r in valid_recs], dtype=object
            ),
            "is_augmented": np.array(
                [((r.meta or {}).get("origin") == "augmented") for r in valid_recs],
                dtype=bool,
            ),
    ...
    def _proto_to_query_records(self, dp: DataProto) -> List[QueryRecord]:
        ...
        for i in range(n):
                    ...
                    self.augmentation_logger.log_annotation(
                        {
                            ...
                            "augmented_text": str(augmented_texts[i])
                            if augmented_texts[i]
                            else None,
                            "solvable": False,
                            "reason": "marked_unsolvable_by_teacher",
                            "estimated_reward": float(est_list[i])
                            if est_list[i] is not None
                            else None,
                        }
                    )
                continue
            ...
            try:
                ...
                final_reward = (
                    np.clip(est_val, -1.0, 1.0) if np.isfinite(est_val) else 0.5
                )
            ...
            record = QueryRecord(
                ...
                teacher_response=str(teacher_responses[i])
                if teacher_responses[i]
                else None,
                meta={
                    "source": "math_dapo",
                    "origin": "augmented",
        ...
        try:
            if isinstance(token_seq, (str, bytes)):
                return (
                    ...
                    else token_seq.decode("utf-8", errors="ignore")
                )
        ...
        try:
            ...
            for i, arr in enumerate(raw_data_list):
                ...
                if key not in key2idx:
                    ...
                    br = (
                        float(base_rewards_arr[i]) if i < len(base_rewards_arr) else 0.5
                    )
            ...
            for idx, br in enumerate(base_rewards_per_original):
                ...
                for _ in range(num_per_prompt):
                    aug_prompts.append(
                        TEMPLATE.format(original_problem=orig_text.strip())
                    )
                    base_rewards_for_aug.append(br)
                    original_texts_for_aug.append(
                        orig_text.strip()
                    )  # save the cleaned problem
            ...
            if (
                "position_ids" not in aug_batch.batch
                and "attention_mask" in aug_batch.batch
            ):
                ...
                if rem != 0:
                    ...
                    if keep <= 0:
                        ...
                        return DataProto(
                            batch=_to_td(
                                {
                                    "input_ids": _torch.zeros(
                                        (0, 0), dtype=_torch.long
                                    ),
                                    "attention_mask": _torch.zeros(
                                        (0, 0), dtype=_torch.long
                                    ),
                                }
                            ),
            ...
            def _extract_sections(gen_txt: str) -> Tuple[str, str]:
                ...
                if m_new:
                    ...
                    diff_src = (
                        m_diff.group("body").strip() if m_diff else t[end:].strip()
                    )
                    return new_problem or t, diff_src or t
            ...
            diff_factors = [
                self._parse_difficulty_strict(t, lo=0.75, hi=1.33) for t in diff_texts
            ]
            ...
            for i in range(min_len):
                ...
                val = float(br) / df if _np.isfinite(df) else br
                val = float(_np.clip(val, -1.0, 1.0)) if _np.isfinite(val) else 0.5
                est_rewards.append(val)

                rec = {
                    "record_id": str(uuid.uuid4()),
                    "original_text": original_texts_for_aug[i]
                    if i < len(original_texts_for_aug)
                    else "",
                    "original_index": original_indices[i]
                    if i < len(original_indices)
                    else 0,
                    "augmented_text": new_texts[i],
                    "full_generation": full_generations[i]
                    if i < len(full_generations)
                    else "",
                    "base_reward": br,
                    ...
                    "text_length_original": len(original_texts_for_aug[i])
                    if i < len(original_texts_for_aug)
                    else 0,
            ...
            nt = {
                ...
                "original_text": _np.array(
                    original_texts_for_aug[:min_len], dtype=object
                ),
            ...
            logger.error(f"Error in generate_augmented_queries: {e}", exc_info=True)
            if self.augmentation_logger:
            ...
            try:
                ...
                for idx, it in enumerate(new_items):
                    ...
                    integration_record = {
                        ...
                        "estimated_reward": float(est_arr[idx])
                        if idx < len(est_arr) and np.isfinite(est_arr[idx])
                        else None,
                ...
                logger.error(f"Failed to integrate teacher batch: {e}", exc_info=True)
    ...
    def _prepare_reward_model_inputs(self, dp: DataProto) -> None:
        ...
        if "reward_model" in nt:
            ...
            gt_candidates = None
            for k in ("teacher/gt", "driver_gt", "gt", "answers", "labels"):
            ...
            if gt_candidates is None:
                ...
                gt_candidates = [
                    (ei.get("raw_answer") if isinstance(ei, dict) else None)
                    for ei in extra_info_list
                ]

            rm_list = []
            for i in range(n):
                rm_list.append({"ground_truth": gt_candidates[i], "style": None})
        ...
        solv_list = [bool(x) if x is not None else True for x in solv_list]
    ...
    def fit(self):
        ...
        self.query_pool = ThreadSafeQueryPool(max_size=max_pool_size)
        sampling_mode = str(
            getattr(self.config.dynamic_data, "sampling_mode", "medium_only")
        ).lower()
        ...
        self._inbox_lock = threading.Lock()
        self._annotated_inbox: deque[DataProto] = deque(maxlen=2000)
        ...
        if dyn_en:
            ...
            init_mode = str(
                getattr(self.config.dynamic_data, "init_mode", "map")
            ).lower()
        ...
        analysis_frequency = getattr(
            self.config.dynamic_data, "analysis_frequency", 500
        )

        try:
            for epoch in range(self.config.trainer.total_epochs):
                self.current_epoch = epoch
                steps_per_epoch = getattr(self.config.trainer, "steps_per_epoch", None)
                ...
                for batch_dict in data_iterable:
                    ...
                    _wanted_nt = ("raw_prompt_ids", "raw_prompt_data", "driver_reward")
                    _nt_to_pop = [
                    ...
                    if (
                        "position_ids" not in gen_batch.batch
                        and "attention_mask" in gen_batch.batch
                    ):
                    ...
                    with marked_timer("step", timing_raw):
                        ...
                        if (
                            ...
                            with marked_timer("gen_max", timing_raw, "red"):
                                ...
                                new_batch.batch["reward_baselines"] = (
                        ...
                        with marked_timer("reward", timing_raw, "yellow"):
                            ...
                            dump_path = getattr(
                                self.config.dynamic_data,
                                "origin_reward_dump_path",
                                "./logs/origin_rewards.jsonl",
                            )
                        ...
                        if do_augment:
                            try:
                                ...
                                want_per_prompt = int(aug_cfg.get("num_per_prompt", 2))
                                ...
                                try:
                                    ...
                                    try:
                                        ...
                                        rp = list(
                                            gen_batch.non_tensor_batch.get(
                                                "raw_prompt_data", []
                                            )
                                        )
                                ...
                                if (
                                    ...
                                    if (
                                        self.teacher_annotator is not None
                                        and len(aug_proto.batch["input_ids"]) > 0
                                    ):
                                        if not self.teacher_annotator.enqueue_aug(
                                            aug_proto
                                        ):
                                            metrics[
                                                "augmentation/queue_full_events"
                                            ] = (
                                                metrics.get(
                                                    "augmentation/queue_full_events", 0
                                                )
                                                + 1
                                            )
                                else:
                                    # Log why we skipped (helps debugging)
                                    metrics["augmentation/skipped_due_to_capacity"] = (
                                        metrics.get(
                                            "augmentation/skipped_due_to_capacity", 0
                                        )
                                        + 1
                                    )
                                    ...
                                    metrics["augmentation/num_per_prompt"] = (
                                        want_per_prompt
                                    )
                        ...
                        # Step 8: return some moderate-difficulty data to the pool for continued training (borrow-learn-return)
                        # if dyn_en:
                        #     try:
                        #         used_ids = set(
                        #             map(str, batch.non_tensor_batch["record_ids"]))
                        #         keep_ids, pri_map = self._compute_moderate_return_plan(
                        #             batch,
                        #             band=float(
                        #                 getattr(self.config.dynamic_data, "moderate_band", 0.25)),
                        #             return_fraction=float(
                        #                 getattr(self.config.dynamic_data, "return_fraction", 0.20)),
                        #             min_keep=int(
                        #                 getattr(self.config.dynamic_data, "min_keep", 1)),
                        #         )

                        #         keep, retire, untouched = [], [], []
                        #         for it in sampled:
                        #             rid = it.record_id
                        #             meta = it.meta or {}
                        #             meta["visits"] = 1 + \
                        #                 int(meta.get("visits", 0))
                        #             meta["last_seen_step"] = self.global_steps
                        #             it.meta = meta

                        #             if rid not in used_ids:
                        #                 # we never trained on it — don't penalize; just put it back unchanged
                        #                 untouched.append(it)
                        #             elif rid in keep_ids and it.meta["visits"] <= int(getattr(self.config.dynamic_data, "max_visits", 3)):
                        #                 keep.append(it)
                        #             else:
                        #                 retire.append(it)  # intentionally drop

                        #         if keep:
                        #             self.query_pool.return_batch(
                        #                 keep,
                        #                 priority_update=pri_map,
                        #                 ema_alpha=float(
                        #                     getattr(self.config.dynamic_data, "ema_alpha", 0.3)),
                        #             )
                        #         if untouched:
                        #             # reinsert untouched with no priority change
                        #             self.query_pool.return_batch(
                        #                 untouched, priority_update=None)

                        #     except Exception as e:
                        #         logger.warning(
                        #             "[moderate-return] failed to return batch: %s", e, exc_info=True)
                    ...
                    metrics["dynamic/pool_size"] = (
                        self.query_pool.size() if dyn_en else 0
                    )
                    metrics["dynamic/capacity_remaining"] = (
                        self.query_pool.capacity_remaining() if dyn_en else 0
                    )
    ...
    def _cleanup_teacher_annotator(self):
        ...
        if teacher is not None:
            ...
            try:
                ...
                logger.warning(f"Error during teacher annotator shutdown(): {e}")
            try:
                teacher.join(timeout=10.0)
                if teacher.is_alive():
                    logger.warning("Teacher annotator thread did not terminate cleanly")
            except Exception as e:
