...
from .dapo_ray_trainer_HeaPA import RayDAPOTrainer
...
def run_dapo_improved(config) -> None:
    if not ray.is_initialized():
        ...
        # Forward wandb credentials and ensure online mode for Ray workers
        for wandb_var in ["WANDB_API_KEY", "WANDB_ENTITY", "WANDB_PROJECT"]:
            if os.environ.get(wandb_var):
                env_vars[wandb_var] = os.environ[wandb_var]
        # Force online mode unless the caller explicitly set WANDB_MODE=offline
        env_vars["WANDB_MODE"] = os.environ.get("WANDB_MODE", "online")
    ...
    def run(self, config):
        ...
        if os.path.isdir(model_path):
            ...
            try:
                ...
                # For text-only models, AutoProcessor falls back to the tokenizer.
                # Guard: only keep it if it's a true multimodal processor.
