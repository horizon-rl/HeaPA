...
# gen_prompt_bsz must be large enough that ~75% pass filter_groups >= train_prompt_bsz.
# With train_prompt_bsz=512, ~384/512 pass → needs 2nd gen-batch → vllm hybrid engine hangs.
# Keep gen_prompt_bsz=512 so first gen-batch yields ~384 diverse prompts > train_prompt_bsz=256.
gen_prompt_bsz=512
...
CKPTS_DIR=${CKPTS_DIR:-"${PWD}/exp/${project_name}/${exp_name}"}
mkdir -p "${CKPTS_DIR}"

TRAIN_FILE="[\"${PWD}/data/dapo-math-14k-no_chinese-unique.parquet\"]"
TEST_FILE="[\"${PWD}/data/aime-2024-960.parquet\"]"
...
python3 -m recipe.dapo.main_dapo_HeaPA \
...
+trainer.save_rollout_records_freq=1
